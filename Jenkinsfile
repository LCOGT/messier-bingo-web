...
pipeline {
	...
	environment {
		...
		AWS_CREDENTIALS = 'jenkins-publish-messierbingo.lco.global'
		AWS_REGION = 'us-west-2'
		AWS_S3_BUCKET = 'messierbingo.lco.global'
	}
	stages {
		stage('Publish to S3 Bucket') {
			steps {
				script {
					...
					// We only synchronize the contents of the source repository
					// with S3 when we have an explicitly tagged release. Otherwise,
					// we take no action.
					if (env.GIT_TAG_NAME != null && env.GIT_TAG_NAME != "null") {
						...
						withAWS(region: env.AWS_REGION, credentials: env.AWS_CREDENTIALS) {
							...
							for (int i = 0; i < items.size(); i++) {
								def item = "${items[i]}"
								s3Upload(file: item, bucket: env.AWS_S3_BUCKET, path: item)
							}
